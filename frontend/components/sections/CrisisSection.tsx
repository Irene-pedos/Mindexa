...
function FeatureItem({ icon: Icon, title, desc, badge, colorClass }: FeatureItemProps) {
  return (
    <div className="relative flex gap-3.5 p-4.5 rounded-xl border border-border/60 bg-card hover:bg-muted/10 hover:border-border transition-all duration-300 group">
      <div className={`size-10 flex items-center justify-center rounded-lg border shrink-0 transition-all duration-300 group-hover:scale-[1.02] ${colorClass}`}>
        <Icon className="w-5 h-5" />
      </div>
      <div className="space-y-1">
        <div className="flex items-center gap-1.5">
          <h4 className="text-sm md:text-base font-medium text-foreground tracking-tight">
            {title}
          </h4>
