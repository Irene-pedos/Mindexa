...
export function KnowledgeCheckFlow({
  ...
  useEffect(() => {
    async function loadQuestions() {
      setLoading(true);
      try {
        const qList = await studyPlannerApi.generateKnowledgeCheck(sessionId, 5);
        setQuestions(qList);
      } catch (err: any) {
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      const res = await studyPlannerApi.submitKnowledgeCheck(sessionId, answers);
      setReport(res);
        ...
        <CardHeader className="border-b border-border/50 bg-muted/20 px-8 py-6 text-center">
          ...
          <Badge
            ...
            className={cn(
              ...
              isPassing ? "border-emerald-500 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "border-amber-500 bg-amber-500/10 text-amber-600 dark:text-amber-400"
            )}
          >
          ...
          <p className="text-xs text-muted-foreground">Knowledge Check for {topic}</p>
        </CardHeader>
        ...
        <CardContent className="px-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl border border-border/60 bg-muted/20 space-y-1">
              ...
              <p className="text-lg font-bold text-foreground">{report.estimated_confidence_level}%</p>
            </div>
            <div className="p-4 rounded-xl border border-border/60 bg-muted/20 space-y-1">
              <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Questions Evaluated</span>
              <p className="text-lg font-bold text-foreground">{report.total_questions}</p>
            </div>
          </div>
              ...
              <div className="p-4 rounded-xl border border-emerald-500/20 bg-emerald-500/5 space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {report.mastered_concepts.map((c, i) => (
                    <Badge key={i} variant="secondary" className="text-[11px] bg-emerald-500/15 text-emerald-700 dark:text-emerald-300">
                      {c}
                    </Badge>
                  ))}
                </div>
              ...
              <div className="p-4 rounded-xl border border-amber-500/20 bg-amber-500/5 space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {report.weak_concepts.map((c, i) => (
                    <Badge key={i} variant="secondary" className="text-[11px] bg-amber-500/15 text-amber-700 dark:text-amber-300">
                      {c}
                    </Badge>
                  ))}
                </div>
  ...
  return (
    <Card className="border-border/70 bg-card shadow-lg rounded-2xl overflow-hidden">
      <CardHeader className="border-b border-border/50 bg-muted/20 px-6 py-5 space-y-3">
        ...
        <Progress value={((currentIdx + 1) / questions.length) * 100} className="h-1.5 w-full bg-muted" />
