...
export default function LecturerWorkspaceDetail() {
  ...
  if (!workspace)
    return (
      <div className="py-20 text-center uppercase font-bold text-muted-foreground">
        Node NULL
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto space-y-4 p-4 pb-12">
      ...
      <div className="flex items-center justify-between border-b border-border/40 pb-4">
        <div className="flex items-center gap-3 min-w-0">
          <Button
            ...
            size="icon"
            asChild
            className="h-7 w-7 rounded-md border border-border/60 hover:bg-muted/50 shrink-0"
          >
          ...
          <div className="min-w-0">
            <h1 className="text-base font-bold tracking-tight text-foreground/90 truncate uppercase">
              {workspace.title}
            </h1>
            <div className="flex items-center gap-2 mt-0.5">
              <Badge
                variant="outline"
                className="font-bold text-[8px] text-primary/70 border-primary/20 bg-primary/5 uppercase h-3.5 px-1.5 rounded-sm"
              >
                {workspace.code}
              </Badge>
        ...
        <div className="flex gap-1.5">
          <Button
            ...
            className="text-destructive h-7 px-3 font-bold text-[9px] uppercase rounded-md border border-destructive/10"
          >
            Archive Node
          </Button>
          <Button
            asChild
            size="sm"
            className="h-7 px-3 bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-[9px] uppercase rounded-md shadow-none"
          >
            <Link href="/lecturer/assessments/new">
              <Plus className="mr-1 size-3" /> New Assessment
            </Link>
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            ...
            <div className="p-3 px-4 rounded-lg border border-border/50 bg-card/30 flex items-center justify-between">
              <div className="space-y-0">
                ...
                <p className="text-base font-bold text-foreground/80 tabular-nums">
            ...
            <div className="p-3 px-4 rounded-lg border border-border/50 bg-card/30 flex items-center justify-between">
              <div className="space-y-0 min-w-0">
                ...
                <p className="text-[10px] font-bold text-foreground/70 uppercase truncate">
            ...
            <CardHeader className="bg-muted/5 border-b border-border/40 py-2.5 px-4 flex flex-row items-center justify-between">
              ...
              <div className="relative w-48">
                ...
                <Input
                  ...
                  className="h-7 pl-6 text-[10px] font-medium rounded-md border-border/60 bg-white focus:ring-0 uppercase placeholder:text-muted-foreground/30"
                  value={searchTerm}
              ...
              <Table>
                <TableHeader className="bg-muted/[0.02]">
                  <TableRow className="h-8 hover:bg-transparent border-b border-border/40">
                    <TableHead className="text-[8px] font-bold uppercase pl-4 text-muted-foreground/50">
                      Vector ID
                    </TableHead>
                ...
                <TableBody>
                  {filteredRoster.length === 0 ? (
                    <TableRow>
                      <TableCell
                        colSpan={4}
                        className="text-center py-12 text-[9px] font-bold uppercase opacity-20 tracking-widest text-muted-foreground"
                      >
                        Zero nodes detected.
                      </TableCell>
                    </TableRow>
                    ...
                    filteredRoster.map((student) => (
                      <TableRow
                        key={student.id}
                        className="h-10 hover:bg-primary/[0.01] transition-colors border-border/10"
                      >
                        <TableCell className="font-mono text-[9px] font-bold pl-4 text-primary/40">
                          {student.student_id}
                        </TableCell>
                        <TableCell>
                          <div className="flex flex-col">
                            <span className="font-bold text-[11px] uppercase tracking-tight text-foreground/70">
                              {student.name}
                            </span>
                            <span className="text-[9px] text-muted-foreground/40 font-bold truncate max-w-[150px]">
                              {student.email}
                            </span>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] font-bold tabular-nums text-foreground/40">
                              {student.progress}%
                            </span>
                          </div>
                        </TableCell>
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="p-3.5 rounded-lg border border-primary/20 bg-primary/5 space-y-1.5">
            <p className="text-[9px] font-bold text-primary uppercase tracking-[0.2em] mb-2 text-center">
              Node Operations
            </p>
            <Button
              asChild
              className="w-full h-8 px-3 text-[9px] font-bold uppercase rounded bg-primary hover:bg-primary/90 text-primary-foreground shadow-none"
              size="sm"
            >
              <Link
              ...
              >
                New Assessment <ArrowRight className="size-2.5 opacity-40" />
              </Link>
            </Button>
            <Button
              asChild
              variant="outline"
              className="w-full h-8 px-3 text-[9px] font-bold uppercase rounded border-primary/10 hover:bg-primary/[0.02] shadow-none"
              size="sm"
            >
              <Link
              ...
              >
                <Database className="size-3" /> Registry Bank
              </Link>
            </Button>
          </div>
          ...
          <Card className="shadow-none border border-border/60 rounded-lg overflow-hidden bg-white">
            <CardHeader className="flex flex-row items-center justify-between py-2 bg-muted/5 border-b border-border/40 px-4">
              ...
              <Button
                variant="ghost"
                size="icon"
                className="h-6 w-6 rounded-md border border-border/60 hover:bg-primary/5 transition-all shrink-0"
                disabled={uploading}
                onClick={() =>
            ...
            <CardContent className="p-0">
              <ScrollArea
                className={cn(materials.length > 0 ? "h-64" : "h-fit")}
              >
                {materials.length === 0 ? (
                  <div className="text-center py-12 px-4 uppercase font-bold text-[8px] text-muted-foreground/20 italic tracking-widest leading-relaxed">
                    Registry Empty.
                  </div>
                ) : (
                  <div className="divide-y divide-border/10">
                    {materials.map((m) => (
                      <div
                        key={m.id}
                        className="flex items-center justify-between p-3 px-4 hover:bg-muted/[0.02] transition-all group"
                      >
                        <div className="flex items-center gap-2 truncate">
                          <div className="bg-primary/5 border border-primary/5 p-1 rounded text-primary/40">
                            <FileText className="size-3" />
                          </div>
                          <div className="truncate">
                            <p className="text-[10px] font-bold truncate uppercase tracking-tight text-foreground/70">
                              {m.display_name || m.original_filename}
                            </p>
                            <p className="text-[8px] font-bold text-muted-foreground/30 uppercase mt-0.5">
                              {m.file_extension.toUpperCase()} •{" "}
                              {(m.file_size_bytes / 1024).toFixed(0)} KB
                            </p>
                          </div>
                        </div>
                        <Download className="size-2.5 text-primary/20 group-hover:text-primary/60 transition-colors" />
                      </div>
                    ))}
                  </div>
                )}
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] p-0 overflow-hidden rounded-xl border border-border/40 shadow-2xl">
          ...
          <div className="p-5">
            {loadingRecord ? (
              <div className="space-y-3 animate-pulse">
                <div className="h-14 bg-muted/20 rounded-lg border border-border/40" />
              </div>
            ) : record ? (
              <div className="space-y-4">
                <div className="grid grid-cols-4 gap-2 p-3 rounded-lg border border-border/40 bg-muted/[0.01]">
                  <div className="space-y-0">
                    <p className="text-[7px] font-bold uppercase text-muted-foreground/50">
                      Vector ID
                    </p>
                    <p className="text-[10px] font-mono font-bold text-primary/60 uppercase">
                      {record.student_id}
                    </p>
                  </div>
                  <div className="space-y-0">
                    ...
                    <p className="text-[10px] font-bold truncate text-foreground/60">
                      {record.email}
                    </p>
                  </div>
                  <div className="space-y-0">
                ...
                <div className="space-y-2">
                  <h3 className="text-[8px] font-bold uppercase tracking-[0.2em] text-muted-foreground/40 flex items-center gap-1.5">
                    <Activity className="size-2.5" /> Execution History
                  </h3>
                  <div className="rounded-lg border border-border/40 divide-y divide-border/10 overflow-hidden bg-card/10">
                    {record.attempts.length === 0 ? (
                      <div className="p-8 text-center text-[9px] font-bold text-muted-foreground/20 uppercase italic">
                        Zero node data.
                      </div>
                    ) : (
                      record.attempts.map((att) => (
                        <div
                          key={att.id}
                          className="p-2.5 px-4 flex items-center justify-between hover:bg-muted/[0.01] transition-colors"
                        >
                          <div className="space-y-0.5 min-w-0 pr-4">
                            <p className="text-[10px] font-bold uppercase tracking-tight text-foreground/70 truncate">
                          ...
                          <div className="text-right shrink-0">
                            {att.percentage !== null ? (
                              ...
                              <span className="text-[8px] font-bold text-muted-foreground/10 uppercase">
                                EVALUATING
                              </span>
                            )}
                          </div>
          ...
          <div className="p-3 bg-muted/5 border-t border-border/40 flex justify-end gap-2 px-5 pb-5">
            <Button
              ...
              className="h-8 px-4 font-bold text-[9px] uppercase"
            >
              Close Audit
            </Button>
            <Button className="h-8 px-4 rounded-md bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-[9px] uppercase shadow-none">
              Export Ledger
            </Button>
          </div>
        </DialogContent>
      ...
      <Dialog open={archiveDialogOpen} onOpenChange={setArchiveDialogOpen}>
        <DialogContent className="max-w-sm p-0 overflow-hidden rounded-xl border border-border/40 shadow-2xl">
          <div className="bg-destructive/90 p-4 text-white">
            <DialogTitle className="text-xs font-bold uppercase">
              Archive Node
            </DialogTitle>
          </div>
          <div className="p-6">
            <p className="text-[11px] text-muted-foreground font-bold leading-relaxed uppercase tracking-tight">
              Preserve node registry and de-prioritize in active operational
              index.
            </p>
          </div>
          <div className="p-3 bg-muted/5 border-t border-border/40 flex justify-end gap-2 px-6 pb-6">
            ...
            <Button
              variant="destructive"
              size="sm"
              className="h-8 px-6 font-bold text-[9px] uppercase rounded-md shadow-none"
