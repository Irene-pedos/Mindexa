...
function CountdownBadge({ expiresAt }: { expiresAt: string }) {
  ...
  return (
    <Badge
      variant="outline"
      className="text-[10px] bg-emerald-500/10 text-emerald-600 border-emerald-500/20 px-2 py-0.5 flex items-center gap-1 font-mono font-semibold"
    >
...
export default function StudentAssessmentsPage() {
  ...
  const pageSize = 10;
  const [selectedAssessmentForStart, setSelectedAssessmentForStart] = useState<
    any | null
  >(null);
  ...
  const hasNewInCategory = (tab: string) => {
    ...
    if (tab === "active") {
      return assessments.some((a) => {
        ...
        return (
          (cat === "ACTIVE" || cat === "IN_PROGRESS") &&
          notifications.some(
            (n) =>
              (n.reference_id === a.id ||
                n.reference_id === a.student_attempt_id) &&
              !n.is_read,
          )
        );
      });
  ...
  const filteredAssessments = assessments.filter((ass) => {
    ...
    const matchesType =
      filterType === "all" ||
      ass.assessment_type?.toLowerCase() === filterType.toLowerCase();
  ...
  const getStatusInfo = (assessment: any) => {
    ...
    return {
      ...
      color:
        category === "IN_PROGRESS" ? "bg-primary text-primary-foreground" : "",
      available: true,
      icon:
        category === "IN_PROGRESS" ? (
          <span className="size-1.5 rounded-full bg-primary-foreground animate-pulse shrink-0" />
        ) : (
  ...
  const renderAssessmentCard = (assessment: any) => {
    ...
    const remainingAttempts = Math.max(
      0,
      assessment.max_attempts - (assessment.attempts_used || 0),
    );
          ...
          <div className="flex-1 py-3.5 px-5 space-y-3.5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                  ...
                  {category === "IN_PROGRESS" &&
                    assessment.student_attempt_expires_at && (
                      <CountdownBadge
                        expiresAt={assessment.student_attempt_expires_at}
                      />
                    )}
                  {category === "IN_PROGRESS" &&
                    !assessment.student_attempt_expires_at && (
                  ...
                  {isSupervised && (
                    <Badge
                      variant="outline"
                      className="text-[9px] uppercase font-bold px-1.5 h-4.5 flex items-center gap-1 border-primary/20 bg-primary/5 text-primary"
                    >
                  ...
                  {assessment.is_password_protected && (
                    <Badge
                      variant="outline"
                      className="text-[9px] bg-warning/5 text-warning border-warning/15 px-1.5 h-4.5 flex items-center gap-1 font-semibold"
                    >
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground font-medium">
                  ...
                  <span>
                    {assessment.course_code || "GEN-001"} -{" "}
                    {assessment.subject || assessment.course_name || "General"}
                  </span>
              ...
              <Badge
                ...
                className={cn(
                  "text-xs font-semibold px-2.5 py-0.5 rounded-full inline-flex items-center gap-1.5 capitalize shadow-none border",
                  status.color,
                )}
              >
                {status.icon}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-1">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground font-medium">
                ...
                <span>
                  {assessment.attempts_used || 0} / {assessment.max_attempts}{" "}
                  Attempts ({remainingAttempts} left)
                </span>
              </div>
              <div className="flex items-center gap-2 text-xs text-muted-foreground font-medium">
            ...
            <div className="text-[11px] text-muted-foreground/80 font-medium">
              {assessment.late_submission_allowed ? (
                <span className="text-success font-semibold">
                  Late submission is eligible for this assessment.
                </span>
              ) : (
            ...
            category === "VIOLATION" ? (
              ...
              <Button
                ...
                onClick={() => {
                  if (status.available) {
                    if (
                      assessment.assessment_type?.toUpperCase() === "GROUP_WORK"
                    ) {
  ...
  const getEmptyMessage = () => {
    if (filterTab === "active")
      return "No active assessments at the moment. You're all caught up!";
    if (filterTab === "upcoming")
      return "No upcoming assessments scheduled. Check back later.";
    if (filterTab === "submitted")
      return "You haven't submitted any assessments yet.";
    if (filterTab === "missed")
      return "Great! You haven't missed any assessments.";
    if (filterTab === "violations")
      return "No integrity violations recorded. Keep up the honest work!";
    return "No assessments identified in this category.";
  };

  return (
    <div
      data-tour="student-assessments"
      className="space-y-5 w-full mx-auto animate-in fade-in duration-300"
    >
      {/* Precision Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-3 border-b border-border/25">
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground font-medium">
            Manage and track your active, upcoming, and completed academic
            tasks.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
          ...
          <Select value={filterType} onValueChange={setFilterType}>
            ...
            <SelectContent>
              ...
              <SelectItem value="group_work" className="text-xs">
                Group Work
              </SelectItem>
              <SelectItem value="reassessment" className="text-xs">
                Reassessment
              </SelectItem>
            </SelectContent>
          </Select>
        ...
        <TabsList className="h-auto p-1 flex-wrap">
          <TabsTrigger value="active" className="relative">
            ...
            {hasNewInCategory("active") && (
              <span className="absolute top-0.5 right-0.5 size-1.5 rounded-full bg-red-400 animate-pulse" />
            )}
          </TabsTrigger>
      ...
      {total > pageSize && (
        <div className="flex items-center justify-between pt-3 border-t border-border/20">
          <p className="text-xs text-muted-foreground font-medium">
            ...
            {Math.min(page * pageSize, total)} of {total} assessments
          </p>
      ...
      <div className="p-3.5 rounded-xl border border-warning/15 bg-warning/5 flex items-start gap-3 transition-all duration-300">
        ...
        <div className="space-y-0.5">
          <p className="text-xs font-semibold text-warning">
            Integrity Protocol Notice
          </p>
          <p className="text-[11px] text-muted-foreground leading-relaxed max-w-4xl font-medium">
            High-security testing environments enforce strict environment
            locking. Switching tabs or applications will register as a
            violation, which may result in session termination and immediate
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open) setSelectedAssessmentForStart(null);
        }}
      >
        {(() => {
          const typeStr = (
            selectedAssessmentForStart?.assessment_type ||
            selectedAssessmentForStart?.type ||
            ""
          ).toUpperCase();
                ...
                {isOpen ? (
                  <>
                    ...
                    <div className="flex items-start gap-3 p-3 rounded-xl border border-primary/20 bg-primary/5">
                      ...
                      <div className="space-y-0.5 flex-1">
                        <p className="text-xs font-semibold text-foreground">
                          AI Study Assistant Enabled
                        </p>
                        <p className="text-[11px] text-muted-foreground leading-normal font-medium">
                          An interactive AI Study Tutor is available directly in
                          your workspace to explain concepts and search your
                      ...
                      <div className="space-y-0.5 flex-1">
                        <p className="text-xs font-semibold text-foreground">
                          {isPractice
                            ...
                            : "Take-Home Flexibility (Pause & Resume)"}
                        </p>
                        <p className="text-[11px] text-muted-foreground leading-normal font-medium">
                          {isPractice
                    ...
                    <div className="flex items-start gap-3 p-3 rounded-xl border border-blue-500/20 bg-blue-500/5">
                      ...
                      <div className="space-y-0.5 flex-1">
                        ...
                        <p className="text-[11px] text-muted-foreground leading-normal font-medium">
                          Tab switching, copy/paste, window blur, and browser
                          focus changes are not monitored and will not trigger
                          warnings.
                        </p>
                      </div>
                    </div>
                    ...
                    {isHomework && (
                      <div className="flex items-start gap-3 p-3 rounded-xl border border-amber-500/20 bg-amber-500/5">
                        ...
                        <div className="space-y-0.5 flex-1">
                          <p className="text-xs font-semibold text-foreground">
                            Submission Deadline & Auto-Finalize
                          </p>
                          <p className="text-[11px] text-muted-foreground leading-normal font-medium">
                            Ensure all answers are submitted before the
                            assessment window closes. When the deadline arrives,
                            your attempt will automatically finalize and submit.
                          </p>
                        </div>
                      </div>
                    )}
                  ...
                  <>
                    ...
                    <div className="flex items-start gap-3 p-3 rounded-xl border border-border/40 bg-muted/10">
                      ...
                      <div className="space-y-0.5">
                        <p className="text-xs font-semibold text-foreground">
                          Tab and App Switching Prohibited
                        </p>
                        <p className="text-[11px] text-muted-foreground leading-normal font-medium">
                          Switching tabs, opening developer tools, or resizing
                          the window registers as a violation. Multiple
                          violations will cause automatic submission.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3 p-3 rounded-xl border border-border/40 bg-muted/10">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-[11px] text-muted-foreground leading-normal font-medium">
                          Attempt{" "}
                          {Math.min(
                            selectedAssessmentForStart?.max_attempts || 1,
                            (selectedAssessmentForStart?.attempts_used || 0) +
                              1,
                          )}{" "}
                          of {selectedAssessmentForStart?.max_attempts || 1}.
                          Remaining attempts:{" "}
                          {Math.max(
                            0,
                            (selectedAssessmentForStart?.max_attempts || 1) -
                ...
                <Button
                  ...
                  onClick={() => {
                    if (selectedAssessmentForStart) {
                      router.push(
                        `/student/assessments/${selectedAssessmentForStart.id}/take`,
                      );
