...
import { getResultLifecycleSummary, getAssessmentCategory } from "@/lib/grading-architecture";
...
export default function StudentResultsPage() {
      ...
      >
        <div className="min-w-0 space-y-1.5">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            <Badge variant="secondary" className="h-5 px-2 text-xs border-none bg-muted/60 text-muted-foreground">
              {getAssessmentTypeLabel(result.assessment_type)}
            </Badge>
            {result.course_code && (
        ...
        <div className="flex items-center gap-5 mt-4 md:mt-0">
            ...
            {isPublished ? (
              <>
                <div className="flex items-baseline justify-end gap-1.5 tabular-nums">
                  ...
                  <span className="text-xs font-medium text-muted-foreground/50">/ {result.total_marks || "N/A"}</span>
                </div>
                <div className={cn(
              ...
              <>
                <div className="text-sm font-semibold text-muted-foreground">Not published</div>
          ...
          >
            <Link href={`/student/results/${result.id}`} className="flex items-center gap-1.5">
  ...
  return (
    <div data-tour="student-results" className="space-y-5 w-full mx-auto animate-in fade-in duration-300">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <ContextualExplainer topic="held-for-review" variant="pill" label="Held for Review Status" />
          </div>
